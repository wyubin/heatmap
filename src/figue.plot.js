...
figue.centerString = function(str, width){
	var diff = width - str.length ;
	if (diff < 0){return };

	var halfdiff = Math.floor(diff / 2) ;
	return this.repeatChar(" " , halfdiff) + str + this.repeatChar(" " , diff - halfdiff);
}
figue.putString = function(str, width, index){
	var diff = width - str.length ;
	if (diff < 0){return };

	return this.repeatChar (" " , index) + str + this.repeatChar (" " , width - (str.length+index)) ;
}
figue.prettyVector = function(vector){
	var vals = new Array(vector.length) ;
	var precision = Math.pow(10, this.PRINT_VECTOR_VALUE_PRECISION) ;
	for (var i = 0 ; i < vector.length ; i++)
		vals[i] = Math.round(vector[i]*precision)/precision ;
	return vals.join(",")
}
figue.prettyValue = function(value){
	var precision = Math.pow(10, this.PRINT_VECTOR_VALUE_PRECISION) ;
	return String (Math.round(value*precision)/precision) ;
}
// plot function
figue.generateDendogram = function(tree, sep, balanced, withLabel, withCentroid, withDistance){
	var lines = new Array ;
	var centroidstr = this.prettyVector(tree.centroid) ;
	if (tree.isLeaf()) {
		...
		lines.push (this.centerString("|" , len)) ;
		if (withCentroid)
			lines.push (this.centerString(centroidstr , len)) ;
		if (withLabel)
			lines.push (this.centerString(labelstr , len)) ;

	} else {
		var distancestr = this.prettyValue(tree.dist) ;
		var left_dendo = this.generateDendogram(tree.left ,sep, balanced,withLabel,withCentroid, withDistance) ;
		var right_dendo = this.generateDendogram(tree.right, sep, balanced,withLabel,withCentroid,withDistance) ;
		...
		lines.push(this.putString("|" , len , bar_ix)) ;
		if (withCentroid) {
			lines.push(this.putString(centroidstr , len , bar_ix - Math.floor (centroidstr.length / 2))) ; //centerString (centroidstr , len)) ;
		}
		if (withDistance) {
			lines.push(this.putString(distancestr , len , bar_ix - Math.floor (distancestr.length / 2))) ; //centerString (centroidstr , len)) ;
		}
		...
		var hline = this.repeatChar("_" , hlineLen) ;
		lines.push (this.putString(hline, len, left_bar_ix)) ;
		...
		for (var i = 0 ; i < Math.max (left_dendo.length , right_dendo.length) ; i++) {
			...
			if (i < left_dendo.length)
				...
				left = this.repeatChar(" " , left_dendo[0].length) ;

			var right = "" ;
			if (i < right_dendo.length)
				right = right_dendo[i] ;
			else
				right = this.repeatChar(" " , right_dendo[0].length) ;
			lines.push(left + this.repeatChar(" " , sep) + right) ;
			var l = left + this.repeatChar(" " , sep) + right ;
		}
	}
